...
from spear_gui.overlay_system_camera import (
    Rect, RectTween,
...
SETTING_TEXT_DEFS = [
    TextDef(0.165, 0.10, 'CAMERA SETTINGS', 25.0, QColor(255,255,255,0), {
        'open':  Phase([Reset(), TextTween(0.05,0.10, 0.00,2.00, QEasingCurve.OutQuint, QColor(255,255,255,255), 0, 1.0, 25.0, 10, 10)]),
        'close': Phase([         TextTween(0.165,0.10, 0.00,0.50, QEasingCurve.OutQuint, QColor(255,255,255,0),   0, 1.0, 25.0, 10, 10)]),
...
def _slider_texts(x: float, y: float, label: str, unit: str) -> SliderTextDefs:
    ...
    return SliderTextDefs(
        label = TextDef(x=x, y=y, px=-110.0, py=0, text=label, font_size=18.0, color=col_zero, phases={
                'open':     Phase([TextTween(x, y, 0.00, 1.50,      QEasingCurve.OutQuint, col_dim, 1.0, 0.5, 18.0, -15, 0)]),
                'close':    Phase([TextTween(x, y, 0.00, 0.30,      QEasingCurve.InQuint, col_zero, 1.0, 0.5, 18.0, -15, 0)])},
        ...
        min_val = TextDef(x=x, y=y, px=-100.0, py=14.0, text='', font_size=13.0, color=col_zero, phases={
                'open':     Phase([TextTween(x, y, 0.00, 1.50,      QEasingCurve.OutQuint, col_dim, 0.0, 0.0, 13.0, 0, 14)]),
                'close':    Phase([TextTween(x, y, 0.00, 0.30,      QEasingCurve.InQuint, col_zero, 0.0, 0.0, 13.0, 0, 14)])},
        ...
        max_val = TextDef(x=x, y=y, px=100.0, py=14.0, text='', font_size=13.0, color=col_zero, phases={
                'open':     Phase([TextTween(x, y, 0.00, 1.50,      QEasingCurve.OutQuint, col_dim, 1.0, 0.0, 13.0, 0, 14)]),
                'close':    Phase([TextTween(x, y, 0.00, 0.30,      QEasingCurve.InQuint, col_zero, 1.0, 0.0, 13.0, 0, 14)])},
        ...
        current = TextDef(x=0.0, y=0.0, px=0.0, py=-14.0, text='', font_size=13.0, color=col_zero, phases={
                'open':     Phase([TextTween(0.0, 0.0, 0.00, 1.50,  QEasingCurve.OutQuint, col_dim,    0.5, 1.0, 13.0, 0, -14)]),
                'changed':  Phase([TextTween(0.0, 0.0, 0.00, 0.20,  QEasingCurve.OutQuint, col_full,   0.5, 1.0, 13.0, 0, -14)]),
                'reverted': Phase([TextTween(0.0, 0.0, 0.00, 0.20,  QEasingCurve.OutQuint, col_dim,    0.5, 1.0, 13.0, 0, -14)]),
                'close':    Phase([TextTween(0.0, 0.0, 0.00, 0.30,  QEasingCurve.InQuint, col_zero,    0.5, 1.0, 13.0, 0, -14)])},
...
CS_TEXT_DEFS = [
    TextDef(0.165, 0.10, 'DISPLAY MODE', 25.0, QColor(255,255,255,0), {
        'open':  Phase([Reset(), TextTween(0.05,0.10, 0.00,2.00, QEasingCurve.OutQuint, QColor(255,255,255,255), 0, 1.0, 25.0, 10, 10)]),
        'close': Phase([         TextTween(0.165,0.10, 0.00,0.50, QEasingCurve.OutQuint, QColor(255,255,255,0),   0, 1.0, 25.0, 10, 10)]),
    }, bold=True, italic=True, font_family='Oxanium SemiBold', h_align=0, v_align=1.0, uniform_scale=False, always_visible=True),
