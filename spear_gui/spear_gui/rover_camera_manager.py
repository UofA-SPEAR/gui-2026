...
def build_pipeline(source, camera_id, port, exposure, gain):
    ...
    # )
    # gpu h265 path generated by 5.3 codex
    return (
        f"{source} {src_props}"
        f"! queue leaky=2 max-size-buffers=4 "
        f"! nvvidconv "
        f"! video/x-raw(memory:NVMM),format=NV12,framerate=30/1 "
        f"! nvv4l2h265enc maxperf-enable=1 control-rate=1 bitrate={BITRATE * 1000} "
        f"iframeinterval=30 idrinterval=30 num-B-Frames=0 "
    ...
    # gpu av1 path generated by 5.3 codex
    # return (
    #     f"{source} {src_props}"
    #     f"! queue leaky=2 max-size-buffers=4 "
    #     f"! nvvidconv "
    #     f"! video/x-raw(memory:NVMM),format=NV12,framerate=30/1 "
    #     f"! nvv4l2av1enc maxperf-enable=1 bitrate={BITRATE * 1000} "
    #     f"! av1parse "
    #     f"! rtpav1pay pt=96 "
    #     f"! udpsink host={RECEIVER_IP} port={port} sync=false async=false"
    # )
