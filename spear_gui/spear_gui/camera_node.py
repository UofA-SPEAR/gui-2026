...
class CameraNode(Node):
    def __init__(self):
        ...
        self._tween_timer.timeout.connect(self._tick_tweens)
    ...
    def setup_gui(self, parent=None):
        ...
        self._overlay_canvas = OverlayCanvas(self.container)
    ...
    def _tick_tweens(self):
        ...
        for wid, tw in self._tweens.items():
            ...
            if not widget or widget.parent() is None:
                ...
                continue
            tw['elapsed'] += 16
            t = min(1.0, tw['elapsed'] / tw['duration'])
            v = self._ease_out_quint(t)
            sg = tw['start_geom']
            nx = int(sg.x()      + (tw['end_x'] - sg.x())      * v)
