...
from spear_gui.overlay_system import (
    ...
    GradientDef, GradientStop, GradientTween, # GradientDef
...
MAIN_GRADIENT_DEFS = [
    GradientDef(
        ...
        # phases={
        #     P_OPEN: Phase([GradientTween(start=0.0, dur=0.4, ease=QEasingCurve.OutQuint, 
        #         stops=[
...
MAIN_WINDOW_DEFS = [
    ...
    # Since we want these EventDef values to change, we can use buttons to simulate movement of the rover.
    ...
    #     ,
    #     text_defs=[
    #         # TextDef(x=0.50, y=0.80, text='KEY = [<#>]', font_size=32.0, bold=True, color=QColor(171, 151, 247, 255), h_align=0.5, v_align=0.0,
    ...
    #         # TextDef(x=0.50, y=0.85, text='', font_size=32.0, bold=True, color=QColor(171, 151, 247, 255), h_align=0.5, v_align=0.0,
    #         #     text_fn=lambda ctx: _ev['textbox_test_value'].value,
    #         # ),

    #         # TextDef(x=0.50, y=0.90, text='Value: <#>', font_size=32.0, bold=True, color=QColor(171, 151, 247, 255), h_align=0.5, v_align=0.0,
    #         #     text_fn=lambda ctx: ctx[_ev['test_event'].value]['latest'],
    #         # ),

            
    #         # *TextBlock(
    #         #     x=0.01, y=0.01, px=0.0, py=0.0,
    #         #     font_size=11.0,
    #         #     color=QColor(255, 255, 0, 200),
    #         #     h_align=0.0, v_align=0.0,
    #         #     text_fn=lambda ctx: f'FPS: {SYS_FPS.value}  |  {SYS_FRAME_TIME.value}ms',
    #         # ),
    #         *TextBlock(
    #             x=0.01, y=0.03, px=0.0, py=0.0,
    #             font_size=11.0,
    #             color=QColor(255, 255, 0, 200),
    #             h_align=0.0, v_align=0.0,
    #             text_fn=lambda ctx: f'Mouse: {int(SYS_MOUSE_X.value)}, {int(SYS_MOUSE_Y.value)}',
    #         ),
    #         # *TextBlock(
    #         #     x=0.5, y=0.5,
    #         #     px=0.0, py=0.0,
    ...
    #     ],
    #     # textbox_defs=[
    #     #     TextboxDef(poly_def=RectDef(p1=P(0.1, 0.8), p2=P(0.45, 0.9)), text_def=TextDef(x=0.12, y=0.85, h_align=0, v_align=0.5, font_size=30, color=QColor(0, 0, 0, 255), uniform_scale=False), event_out = _ev['textbox_test_value']),
    #     #     TextboxDef(poly_def=RectDef(p1=P(0.55, 0.8), p2=P(0.9, 0.9)), text_def=TextDef(x=0.57, y=0.85, h_align=0, v_align=0.5, font_size=30, color=QColor(0, 0, 0, 255), uniform_scale=False), event_out = _ev['test_event'])
    #     # ],
    #     sub_windows=[
            
    #         WindowDef(
    #             p1=P(0.01, 0.00), p2=P(1.00, 1.00),
    ...
    #             phases={
    #                 P_OPEN:  Phase([WindowTween(p1=P(0.01, 0.00), p2=P(1.00, 1.00), start=0, dur=0.3, ease=QEasingCurve.OutQuint)]),
    #                 '1':     Phase([WindowTween(p1=P(0.01, -0.05), p2=P(1.00, 1.00 - 0.05), start=0, dur=0.3, ease=QEasingCurve.OutQuint)]),
    #                 '2':     Phase([WindowTween(p1=P(0.01, -0.10), p2=P(1.00, 1.00 - 0.10), start=0, dur=0.3, ease=QEasingCurve.OutQuint)]),
    #                 '3':     Phase([WindowTween(p1=P(0.01, -0.15), p2=P(1.00, 1.00 - 0.15), start=0, dur=0.3, ease=QEasingCurve.OutQuint)]),
    #                 '4':     Phase([WindowTween(p1=P(0.01, -0.20), p2=P(1.00, 1.00 - 0.20), start=0, dur=0.3, ease=QEasingCurve.OutQuint)]),
    #                 '5':     Phase([WindowTween(p1=P(0.01, -0.25), p2=P(1.00, 1.00 - 0.25), start=0, dur=0.3, ease=QEasingCurve.OutQuint)]),
    #                 P_CLOSE: Phase([WindowTween(p1=P(0.50, -0.30), p2=P(1.50, 1.00 - 0.30), start=0, dur=0.3, ease=QEasingCurve.InQuint)]),
    #             },
    #             text_defs=[
    #                 TextDef(
    #                     x=0.0, y=0.5,
    ...
    #                     text_fn=_ev['textbox_test_value'],
    #                 )
    #             ],
    #         ),
    ...
    #             spawn_limit=20,
    #             phase_event=GROUP_EVENT,         # patched to group_ev per instance
    ...
    #                     ),
    #                     action='set',
    #                     event_out=GROUP_EVENT,    # patched to group_ev per instance
    #                     event_delta=GROUP_EVENT,  # patched to spawn_delete_threshold
    #                 )
    #             ],
    #         )
            ...
            #     ],
            #     text_defs=[
            #         TextDef(x=0.50, y=0.50, text='TEST', font_size=150.0, bold=True, color=QColor(255, 255, 255, 255), h_align=0.5, v_align=0.5),
            #     ],
            #     sub_windows=[
            #         WindowDef(p1=P(0.40, 0.45), p2=P(0.60, 0.55),
            #             draggable=True, scalable=True,
