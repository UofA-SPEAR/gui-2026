...
tasks_window = WindowDef(
    ...
    phases={
        'open': Phase([WindowTween(p1=P(0.5, 0.0), p2=P(1.0, 1.0), px1=P(157, 0), start=0.0, dur=1.0, ease=QEasingCurve.OutQuint)])
    },
    listener_defs=[
    ...
    text_defs=[
        ...
        TextDef(p=P(1, 0), px=P(15, 95), text='<#>', bold=True, font_size=30, h_align=0.0, v_align=0.0, char_display=0, sub_char_clip=True, fill_color=QColor(255,255,255,0), gradient=get_gradient('alt_color_outline'), outline_width=1, 
    ...
    button_defs=[
        *SegmentedButtons(
            p1=P(0.40, 0.03), p2=P(0.90, 0.06), px1=P(0, 0), px2=P(0, 0), event_out=get_event('task_window'), 
            ...
            segments=[
                ...
                Segment(key=Qt.Key_4, event_delta='3. RoverCooked', weight=1, label='RVK'),
...
hardware_fault_window = WindowDef(
    p1=P(0.5, 0.0), p2=P(1.0, 1.0), px1=P(157, 0), px2=P(-60, 0),
    ...
    text_defs=[
        TextDef(p=P(0.50, 0.42), px=P(0, 30), text='XOR', font_size=16.0, fill_color=QColor(171, 151, 247, 0), h_align=0.5, v_align=0.5, phases={
            'open': Phase([Reset(), TextTween(px=P(0, 0), fill_color=QColor(171, 151, 247, 255), start=0.3, dur=1, ease=QEasingCurve.OutQuint)]),
...
camera_access_window = WindowDef(
    p1=P(0.5, 0.0), p2=P(1.0, 1.0), px1=P(157, 0),
    ...
    listener_defs=[
        # 3 WIRE CONDITIONS
        EventListener(targets = [get_event('wire_c3-1')], value_fn = lambda ctx: get_event('wire_red_count').value,                                     conditions = [lambda v: v == 0],                         values = [True, False]),
        EventListener(targets = [get_event('wire_c3-2')], value_fn = lambda ctx: get_event('wire_last_color').value,                                    conditions = [lambda v: v == 'white'],                   values = [True, False]),
        EventListener(targets = [get_event('wire_c3-3')], value_fn = lambda ctx: get_event('wire_blue_count').value,                                    conditions = [lambda v: v > 1],                          values = [True, False]),
        # 4 WIRE CONDITIONS
        EventListener(targets = [get_event('wire_c4-1')], value_fn = lambda ctx: get_event('wire_red_count').value,                                     conditions = [lambda v: v > 1],                          values = [True, False]),
        EventListener(targets = [get_event('wire_c4-2')], value_fn = lambda ctx: (get_event('wire_last_color').value, get_event('wire_red_count').value),     conditions = [lambda v: v[0] == 'yellow' and v[1] == 0], values = [True, False]),
        EventListener(targets = [get_event('wire_c4-3')], value_fn = lambda ctx: get_event('wire_blue_count').value,                                    conditions = [lambda v: v == 1],                         values = [True, False]),
        EventListener(targets = [get_event('wire_c4-4')], value_fn = lambda ctx: get_event('wire_yellow_count').value,                                  conditions = [lambda v: v > 1],                          values = [True, False]),
        # 5 WIRE CONDITIONS
        EventListener(targets = [get_event('wire_c5-1')], value_fn = lambda ctx: get_event('wire_last_color').value,                                    conditions = [lambda v: v == 'black'],                   values = [True, False]),
        EventListener(targets = [get_event('wire_c5-2')], value_fn = lambda ctx: (get_event('wire_red_count').value, get_event('wire_yellow_count').value),   conditions = [lambda v: v[0] == 1 and v[1] > 1],         values = [True, False]),
        EventListener(targets = [get_event('wire_c5-3')], value_fn = lambda ctx: get_event('wire_black_count').value,                                   conditions = [lambda v: v == 0],                         values = [True, False]),
        # 6 WIRE CONDITIONS
        EventListener(targets = [get_event('wire_c6-1')], value_fn = lambda ctx: get_event('wire_yellow_count').value,                                  conditions = [lambda v: v == 0],                         values = [True, False]),
        EventListener(targets = [get_event('wire_c6-2')], value_fn = lambda ctx: (get_event('wire_yellow_count').value, get_event('wire_white_count').value), conditions = [lambda v: v[0] == 1 and v[1] > 1],         values = [True, False]),
        EventListener(targets = [get_event('wire_c6-3')], value_fn = lambda ctx: get_event('wire_red_count').value,                                     conditions = [lambda v: v == 0],                         values = [True, False]),
    ],
    polygon_defs=[
        ...
        RectDef(p1=P(0.68, 0.58), p2=P(0.82, 0.82), fill_color=QColor(171, 151, 247, 0), outline_color=QColor(171, 151, 247, 0), outline_width=1.0, phases={
    ...
    text_defs=[
        ...
        # INSTRUCTIONS
        TextDef(p=P(0.50, 0.90), text='CUT THE FIRST WIRE', font_size=32.0, bold=True, fill_color=QColor(171, 151, 247, 255), h_align=0.5, v_align=0.0, char_display=0.0, sub_char_clip=True,
            phase_override=lambda: (
                'open' if get_event('wire_count').value == 4 and not get_event('wire_c4-1').value and (get_event('wire_c4-2').value or get_event('wire_c4-3').value) else 
                'open' if get_event('wire_count').value == 5 and not get_event('wire_c5-1').value and get_event('wire_c5-2').value else 
                'close'
            ), phases={
        ...
        TextDef(p=P(0.50, 0.90), text='CUT THE SECOND WIRE', font_size=32.0, bold=True, fill_color=QColor(171, 151, 247, 255), h_align=0.5, v_align=0.0, char_display=0.0, sub_char_clip=True,
            phase_override=lambda: (
                'open' if get_event('wire_count').value == 3 and get_event('wire_c3-1').value else 
                'open' if get_event('wire_count').value == 4 and not get_event('wire_c4-1').value and not get_event('wire_c4-2').value and not get_event('wire_c4-3').value and not get_event('wire_c4-4').value else 
                'open' if get_event('wire_count').value == 5 and not get_event('wire_c5-1').value and not get_event('wire_c5-2').value and get_event('wire_c5-3').value else 
                'open' if get_event('wire_count').value == 6 and not get_event('wire_c6-1').value and not get_event('wire_c6-2').value and not get_event('wire_c6-3').value else 
                'close'
            ), phases={
                P_OPEN:  Phase([Reset(), TextTween(char_display=1.0, start=0, dur=0.5, ease=QEasingCurve.OutQuint)]),
                P_CLOSE: Phase([         TextTween(char_display=0.0, h_align=-0.5, start=0, dur=0.5, ease=QEasingCurve.OutQuint)])},
        ),
        TextDef(p=P(0.50, 0.90), text='CUT THE THIRD WIRE', font_size=32.0, bold=True, fill_color=QColor(171, 151, 247, 255), h_align=0.5, v_align=0.0, char_display=0.0, sub_char_clip=True,
            phase_override=lambda: (
                'open' if get_event('wire_count').value == 6 and get_event('wire_c6-1').value else 
                'close'
            ), phases={
                P_OPEN:  Phase([Reset(), TextTween(char_display=1.0, start=0, dur=0.5, ease=QEasingCurve.OutQuint)]),
                P_CLOSE: Phase([         TextTween(char_display=0.0, h_align=-0.5, start=0, dur=0.5, ease=QEasingCurve.OutQuint)])},
        ),
        TextDef(p=P(0.50, 0.90), text='CUT THE FOURTH WIRE', font_size=32.0, bold=True, fill_color=QColor(171, 151, 247, 255), h_align=0.5, v_align=0.0, char_display=0.0, sub_char_clip=True,
            phase_override=lambda: (
                'open' if get_event('wire_count').value == 5 and get_event('wire_c5-1').value else 
                'open' if get_event('wire_count').value == 6 and not get_event('wire_c6-1').value and get_event('wire_c6-2').value else 
                'close'
            ), phases={
                P_OPEN:  Phase([Reset(), TextTween(char_display=1.0, start=0, dur=0.5, ease=QEasingCurve.OutQuint)]),
                P_CLOSE: Phase([         TextTween(char_display=0.0, h_align=-0.5, start=0, dur=0.5, ease=QEasingCurve.OutQuint)])},
        ...
        TextDef(p=P(0.50, 0.90), text='CUT THE LAST WIRE', font_size=32.0, bold=True, fill_color=QColor(171, 151, 247, 255), h_align=0.5, v_align=0.0, char_display=0.0, sub_char_clip=True,
            phase_override=lambda: (
                'open' if get_event('wire_count').value == 3 and ((not get_event('wire_c3-1').value and get_event('wire_c3-2').value) or (not get_event('wire_c3-1').value and not get_event('wire_c3-2').value and not get_event('wire_c3-3').value)) else 
                'open' if get_event('wire_count').value == 4 and not get_event('wire_c4-1').value and not get_event('wire_c4-2').value and not get_event('wire_c4-3').value and get_event('wire_c4-4').value else 
                'open' if get_event('wire_count').value == 5 and not get_event('wire_c5-1').value and not get_event('wire_c5-2').value and not get_event('wire_c5-3').value else 
                'open' if get_event('wire_count').value == 6 and not get_event('wire_c6-1').value and not get_event('wire_c6-2').value and get_event('wire_c6-3').value else 
                'close'
            ), phases={
                P_OPEN:  Phase([Reset(), TextTween(char_display=1.0, start=0, dur=0.5, ease=QEasingCurve.OutQuint)]),
                P_CLOSE: Phase([         TextTween(char_display=0.0, h_align=-0.5, start=0, dur=0.5, ease=QEasingCurve.OutQuint)])},
        ...
        TextDef(p=P(0.50, 0.90), text='CUT THE LAST RED WIRE', font_size=32.0, bold=True, fill_color=QColor(171, 151, 247, 255), h_align=0.5, v_align=0.0, char_display=0.0, sub_char_clip=True,
            phase_override=lambda: (
                'open' if get_event('wire_count').value == 4 and get_event('wire_c4-1').value else 
                'close'
            ), phases={
                P_OPEN:  Phase([Reset(), TextTween(char_display=1.0, start=0, dur=0.5, ease=QEasingCurve.OutQuint)]),
                P_CLOSE: Phase([         TextTween(char_display=0.0, h_align=-0.5, start=0, dur=0.5, ease=QEasingCurve.OutQuint)])},
        ),
        TextDef(p=P(0.50, 0.90), text='CUT THE LAST BLUE WIRE', font_size=32.0, bold=True, fill_color=QColor(171, 151, 247, 255), h_align=0.5, v_align=0.0, char_display=0.0, sub_char_clip=True,
            phase_override=lambda: (
                'open' if get_event('wire_count').value == 3 and not get_event('wire_c3-1').value and not get_event('wire_c3-2').value and get_event('wire_c3-3').value else 
                'close'
            ), phases={
                P_OPEN:  Phase([Reset(), TextTween(char_display=1.0, start=0, dur=0.5, ease=QEasingCurve.OutQuint)]),
                P_CLOSE: Phase([         TextTween(char_display=0.0, h_align=-0.5, start=0, dur=0.5, ease=QEasingCurve.OutQuint)])},
    ...
    button_defs=[
        *SegmentedButtons(
            p1=P(1.00, 0.18), p2=P(1.00, 0.22), px1=P(-480, 0), px2=P(-80, 0), event_out=get_event('wire_count'), segments=[
        ...
        *SegmentedButtons(
            p1=P(1.00, 0.28), p2=P(1.00, 0.32), px1=P(-480 + 40, 0), px2=P(-80 - 40, 0), event_out=get_event('wire_red_count'), 
            phase_override=lambda: (
                'open' if get_event('wire_count').value == 3 else 
                'open' if get_event('wire_count').value == 4 else 
                'open' if get_event('wire_count').value == 5 and not get_event('wire_c5-1').value else 
        ...
        *SegmentedButtons(
            p1=P(1.00, 0.33), p2=P(1.00, 0.37), px1=P(-480 + 40, 0), px2=P(-80 - 40, 0), event_out=get_event('wire_yellow_count'), 
            phase_override=lambda: (
                'open' if get_event('wire_count').value == 4 and not get_event('wire_c4-1').value and not get_event('wire_c4-2').value and not get_event('wire_c4-3').value else
                'open' if get_event('wire_count').value == 5 and not get_event('wire_c5-1').value else 
                'open' if get_event('wire_count').value == 6 else 
        ...
        *SegmentedButtons(
            p1=P(1.00, 0.38), p2=P(1.00, 0.42), px1=P(-480 + 40, 0), px2=P(-80 - 40, 0), event_out=get_event('wire_blue_count'), 
            phase_override=lambda: (
                'open' if get_event('wire_count').value == 3 and not get_event('wire_c3-1').value and not get_event('wire_c3-2').value else 
                'open' if get_event('wire_count').value == 4 and not get_event('wire_c4-1').value and not get_event('wire_c4-2').value else
                'close'
        ...
        *SegmentedButtons(
            p1=P(1.00, 0.43), p2=P(1.00, 0.47), px1=P(-480 + 40, 0), px2=P(-80 - 40, 0), event_out=get_event('wire_white_count'), 
            phase_override=lambda: (
                'open' if get_event('wire_count').value == 6 and not get_event('wire_c6-1').value else
                'close'
            ), segments=[
        ...
        *SegmentedButtons(
            p1=P(1.00, 0.48), p2=P(1.00, 0.52), px1=P(-480 + 40, 0), px2=P(-80 - 40, 0), event_out=get_event('wire_black_count'), 
            phase_override=lambda: (
                'open' if get_event('wire_count').value == 5 and not get_event('wire_c5-1').value and not get_event('wire_c5-2').value else
                'close'
            ), segments=[
        ...
        *SegmentedButtons(
            p1=P(1.00, 0.53), p2=P(1.00, 0.57), px1=P(-480, 0), px2=P(-80, 0), event_out=get_event('wire_last_color'), 
            phase_override=lambda: (
                'open' if get_event('wire_count').value == 3 and not get_event('wire_c3-1').value else
                'open' if get_event('wire_count').value == 4 and not get_event('wire_c4-1').value else
                'open' if get_event('wire_count').value == 5 else
        ...
        # RESET VALUES
        *SegmentedButtons(
            p1=P(1.00, 0.58), p2=P(1.00, 0.62), px1=P(-480 + 120, 0), px2=P(-80 - 120, 0), event_out=get_event('wire_last_color'), 
            phase_override=lambda: (
                ...
                Segment(key=Qt.Key_3, event_delta='wawa', weight=1, label='RESET'),
            ],
        ),
    ],
)
